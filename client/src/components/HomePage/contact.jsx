...
function Contact() {
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      title: 'Email',
      content: 'kelurahan@karangpucung.pwt.go.id',
      color: 'text-red-600',
    },
    {
  ...
  return (
    <Element name="kontak">
      <section className="bg-black py-15" id="contact">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {contactInfo.map((info, index) => (
                  <div
                    key={index}
                    className="bg-gray-900 border border-gray-700 rounded-xl p-6 transition-all duration-300 hover:border-green-600 hover:shadow-lg hover:shadow-green-600/20 transform hover:-translate-y-1 px-5 cursor-pointer group"
                    onMouseEnter={() => setHoveredCard(`info-${index}`)}
                    onMouseLeave={() => setHoveredCard(null)}
                  >
                    <div className={`${info.color} mb-3 transition-transform duration-300 ${hoveredCard === `info-${index}`}`}>{info.icon}</div>
